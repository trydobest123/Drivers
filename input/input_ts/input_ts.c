...
{	
	...
	{	
		mod_timer(&adc_timer,jiffies+msecs_to_jiffies(20));
	}
...
/* 让内核线程处理该函数 */
static void ts_work_func(unsigned int data)
{
	...
	if(!CheckPenState())
	{
		...
		mod_timer(&adc_timer,jiffies+msecs_to_jiffies(5));
		TsWaitRelaease();
	}
	else
	{
