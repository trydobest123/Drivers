...
static void camera_ov9650_init(void)
{
	...
	mid   =  cmos_ov9650_read_byte(0xa) << 8;
	mid |= cmos_ov9650_read_byte(0xb);
	...
	{
		cmos_ov9650_write_byte(ov9650_reg_set[i].addr,ov9650_reg_set[i].value);
		mdelay(10);
	}
}
...
static int cmos_ov9650_init(void)
{
		int ret;
...
static void cmos_ov9650_exit(void)
{
	camif_gpio_exit();
	camif_reg_exit();
...
module_init(cmos_ov9650_init);
module_exit(cmos_ov9650_exit);
MODULE_LICENSE("GPL");
